...
import { fetchBackPageDataByVideoId, fetchLastVideoFromPreviousSection, fetchNextSectionVideos } from '@/app/_api/videos';
import { fetchLastVideoFromPreviousDetailedTopicSection, fetchNextDetailedTopicSectionVideos } from '@/app/_api/detailed-topic-sections-videos';
...
export default async function Question({
    ...
    video = (dataArr && dataArr.length > 0 && dataArr[0].videos) ? dataArr[0].videos : null;
    ...
    if (backPageData) {
        ...
        if (!isDetailedTopic) {
            ...
            otherVideosRes = await fetchVideosByDetailedTopicSectionId(currSectionId, user?.id)
            ...
            if (!fromPrivate && currSectionId) {
                ...
                if (sectionData && sectionData.topic_id) {
                    const detailedTopic = await fetchDetailedTopicById(sectionData.topic_id);
        ...
        currVideoIdx = otherVideosRes.findIndex((otherVideoObj: { video_id: any }) => otherVideoObj.video_id === video?.id);
        ...
        if (prevVideo === null && !isDetailedTopic) {
            prevVideo = await fetchLastVideoFromPreviousSection(currTopicId, user?.id, currSectionId, currSectionOrder);
        }
        else if (prevVideo === null && isDetailedTopic) {
            prevVideo = await fetchLastVideoFromPreviousDetailedTopicSection(currSectionId, sectionName, currTopicId, currSectionOrder);
        }

        // If there is no videos left in the section go to the next
        if (nextVideos.length === 0 && !isDetailedTopic) {
            nextVideos = await fetchNextSectionVideos(currSectionId, currTopicId, currSectionOrder, user?.id);
            ...
            nextVideos = await fetchNextDetailedTopicSectionVideos(currSectionId, user?.id, currTopicId, currSectionOrder);
            if (nextVideos[0]) {
                nextSectionId = nextVideos[0].section_id;
                const nextSectionData = await fetchDetailedTopicSectionById(nextSectionId);
                nextSectionName = nextSectionData.name;
            }
        }
    }
    ...
    return (
        <div className="flex flex-col lg:flex-row lg:space-x-4 max-sm:space-y-4 items-start">
            ...
            <div className={`w-full ${!hideSidebar ? 'lg:w-2/3' : 'lg:w-full'} p-4 flex flex-col`}>
                {video && <div className='flex md:items-center md:flex-row flex-col justify-center'>
                    <div className='md:flex-1 hidden md:block'>
                        <Link
                            href={!backPageData ? '/dashboard/home' : backPageData.sections ? `/dashboard/home/${backPageData.sections.topics.id}` : `/dashboard/topics/info/${backPageData.detailed_topics_sections.detailed_topics.id}`}
                            className='contents'>
                    ...
                    <div className='flex gap-2 w-full max-w-[400px] items-center'>
                        {video.doctors && <Link href={`/dashboard/doctors/${video.doctors.id}`} className='bg-card-background-primary-gradient rounded-full flex py-2 pl-2 pr-4 text-white gap-2 mx-auto items-center'>
                            <img
                                src={video.doctors.picture_url}
                                alt="Doctor Picture"
                                className="w-10 h-10 md:w-16 md:h-16 rounded-full object-cover" />
                            <div className='flex flex-col w-full gap-2'>
                                <p className='text-sm md:text-xl font-semibold'>{video.doctors.name}</p>
                                <div className='flex justify-between mr-4 md:text-md text-xs'>
                                    <p className='mr-1'>{video.doctors.specialty}</p>
                                    {video.doctors.city && video.doctors.state && <p>{video.doctors.city}, {video.doctors.state}</p>}
                                    {video.doctors.city && !video.doctors.state && <p>{video.doctors.city}</p>}
                                    {!video.doctors.city && video.doctors.state && <p>{video.doctors.state}</p>}
                                </div>
                            </div>
                        </Link>}
                ...
                {video && <VideoBody prevVideo={prevVideo} video={video} nextVideos={nextVideos} backPageData={backPageData} />}
            ...
            {video && <div className="w-full lg:w-1/3 p-4 lg:h-screen lg:overflow-y-auto">
                <OtherVideos results={nextVideos} topicName={topicName} sectionName={sectionName} currVideoIdx={currVideoIdx} nextSectionName={nextSectionName} videoTotal={nextVideoTotal} isDetailed={isDetailedTopic} topicId={currTopicId} sectionId={currSectionId} nextSectionId={nextSectionId} />
            </div>}
