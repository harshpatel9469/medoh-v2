...
export default function Treatments({ params: { id } }: any) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const publicTopics = topicsData.filter(
        (topic) => topic.is_private === false
      ); // ✅ Filter private
  ...
  const handleFilter = (filter: Filter | null) => {
    if (filter === null) {
      ...
      setFilteredTopics(
        allTopics.filter((topic, index) => topic.filter_id === filter.id)
      );
    ...
    <div>
      {isLoading ? (
        ...
        <div className="mx-4">
          ...
          {filters.length !== 0 && (
            <div className="flex gap-4 mb-6 flex-nowrap overflow-x-auto text-nowrap p-4 md:p-2">
              <p
                className={
                  selectedFilter === null
                    ? "py-1 px-2 bg-card-background-primary-gradient rounded-full font-semibold text-white hover:cursor-pointer ring-2 ring-offset-2 ring-primary-color"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {filteredTopics.map((topic, index) => (
              <Link
                ...
                style={{
                  backgroundImage: `url(${topic.image_url}), var(--card-background-primary-gradient)`,
                }}
              ...
              >
                <span className="text-lg font-semibold text-white bg-card-background-primary-gradient py-1 px-2 rounded-full text-center">
                  {topic.name}
                </span>
              </Link>
            ))}
