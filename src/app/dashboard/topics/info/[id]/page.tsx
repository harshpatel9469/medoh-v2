...
import {
  ...
  fetchAllDetailedTopics,
} from "@/app/_api/detailed-topics";
import {
  fetchDetailedTopicSections,
  partialFetchVideosByDetailedTopicSectionId,
...
function DetailedTopic({ params: { id } }: any) {
  const [detailedTopic, setDetailedTopic] = useState<DetailedTopic | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const versions = allTopics.filter(
        (t) => stripLanguageSuffix(t.name) === baseName
      );
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      for (const section of sections) {
        const sectionVideos = await partialFetchVideosByDetailedTopicSectionId(
          section.id,
          userId
        );
  ...
  return (
    <div className="flex-grow">
      <div className="md:mx-4 mx-1">
        ...
        {detailedTopic && (
          <div className="bg-card-background-primary-gradient w-full flex text-white rounded-xl shadow-sm items-center justify-between">
            <div className="flex flex-col m-4 gap-2 w-full md:w-2/3 items-center md:items-start justify-center">
              <Link
                className="flex gap-1 items-center font-bold mb-6 self-start"
                href={`/dashboard/topics/${detailedTopic.topic_id}`}
              >
                <p>&lt;</p>
                <p>Back</p>
              </Link>
                  ...
                  {languageVersions.map((ver) => (
                    <Link
                      ...
                      className={`px-3 py-1 rounded-full border text-sm font-semibold transition-colors ${
                        getLanguageFromName(ver.name) === currentLanguage
                          ? "bg-white text-primary-color border-primary-color"
                          : "bg-transparent text-white border-white hover:bg-white hover:text-primary-color"
                      }`}
                    >
        ...
        {sections && videos && (
          <div className="grid auto-rows-auto grid-cols-1 gap-6 mt-6">
            {sections.map((section) => (
              <div key={section.id}>
                <Link
                  href={`/dashboard/topics/details/${section.id}`}
