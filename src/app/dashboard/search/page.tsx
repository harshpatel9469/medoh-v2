...
import { searchContent, searchContentWithCondition } from '@/app/_api/search-actions';
import SearchBar from "@/app/_components/forms/search-bar";
import LoadingSpinner, { SearchResultsSkeleton } from "@/app/_components/loading-spinner";
...
export default function Home() {
    ...
    const [activeTab, setActiveTab] = useState<'questions' | 'topics' | 'experts'>('questions');
    ...
    const [groupedCardIndexes, setGroupedCardIndexes] = useState<{ [key: number]: number }>({});
    const [activeCondition, setActiveCondition] = useState<string>('');

    // Use content store for recent searches
    const { recentSearches, setRecentSearches, topics, setTopics } = useContentStore();
    ...
    const getCurrentHealthConcerns = useCallback(() => {
        ...
        if (user?.user_metadata?.health_concerns && user.user_metadata.health_concerns.length > 0) {
    ...
    const performSearch = useCallback(async (term: string, condition: string, searchId: number) => {
        if (!term || term.trim().length < 1) {
            return { searchResults: [], topicSearchResults: [], sectionSearchResults: [] };
        }
        ...
        const searchPromise = (async () => {
            try {
                console.log('Making API request for:', searchKey, 'searchId:', searchId);
                let searchResults: any[] = [], topicSearchResults: any[] = [], sectionSearchResults: any[] = [];
                ...
                const result = { searchResults, topicSearchResults, sectionSearchResults };
    ...
    const getSectionDetailsForSearchResults = useCallback(async (searchResults: any[]) => {
        ...
        const uniqueSectionIds = sectionIdsInOrder.filter((id, index) => 
            sectionIdsInOrder.indexOf(id) === index
        );
        ...
        const sectionMap = new Map((data || []).map(section => [section.id, section]));
    ...
    const handleSearch = useDebouncedCallback(async (term: string) => {
        console.log('Search called with term:', term, 'condition:', activeCondition, 'searchTerm:', searchTerm);
        ...
        setLoading(true);
        console.log('Setting loading to true for search:', term, 'searchId:', searchId);
        
        try {
            const { searchResults, topicSearchResults, sectionSearchResults } = await performSearch(term, activeCondition, searchId);
            console.log('Search results:', searchResults);
            ...
            if (searchId === currentSearchId) {
                ...
                const sectionDetails = await getSectionDetailsForSearchResults(searchResults);
                ...
                console.log('Skipped updating results for outdated searchId:', searchId, 'current:', currentSearchId);
            }
        } catch (error) {
            ...
            if (searchId === currentSearchId) {
                setLoading(false);
                console.log('Setting loading to false for search:', term, 'searchId:', searchId);
            }
        }
    ...
    const groupedResults = useMemo(() => groupResultsByQuestion(results), [results]);

    // Helper to render grouped result card
    const renderGroupedResultCard = useCallback((group: any[], groupIdx: number) => {
      ...
      const handlePrev = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.stopPropagation();
        setGroupedCardIndexes(idx => ({ ...idx, [groupIdx]: (currentIdx - 1 + total) % total }));
      };
      const handleNext = (e: React.MouseEvent<HTMLButtonElement>) => {
        e.stopPropagation();
        setGroupedCardIndexes(idx => ({ ...idx, [groupIdx]: (currentIdx + 1) % total }));
      };
      return (
        <div
          key={groupIdx}
        ...
        >
          <div className="text-xl font-bold mb-2">{item.content || item.question_text || item.name}</div>
          ...
          <div className="flex items-center justify-between mt-auto">
            ...
            {showArrowsAndCount && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs">{currentIdx + 1} / {total} ({total} doctor{total > 1 ? 's' : ''})</span>
    ...
    useEffect(() => {
        ...
        setLoading(false);
    }, [searchParams, recentSearches.length, setRecentSearches, topics.length, setTopics, searchTerm, handleSearch])
        ...
        <div className="min-h-screen p-6 flex justify-center">
            <div className="w-full max-w-full pw-10">
                ...
                {healthConcerns.length > 0 ? (
                    ...
                    healthConcerns.map((condition: string) => (
                        <div key={condition} className={activeCondition === condition ? 'block' : 'hidden'}>
                            <SearchBar 
                    ...
                    {searchTerm !== '' && (
                        <div>
                            ...
                            <div className="flex gap-2 mb-6">
                                ...
                                >
                                    {activeCondition && activeCondition !== 'General' ? 'Topics' : 'Topics'}
                                </button>
                                <button
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                                        activeTab === 'experts' 
                            ...
                            {activeTab === 'questions' && (
                                <>
                                    <h2 className="text-lg font-semibold mb-4">Relevant Questions</h2>
                                    {loading && <SearchResultsSkeleton />}
                                    {!loading && results.length > 0 && (
                                        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
                                            {groupedResults.map((group, idx) => renderGroupedResultCard(group, idx))}
                                        </div>
                                    )}
                                    {!loading && results.length === 0 && (
                                        <div className='flex flex-col w-full items-center'>
                                            <h3 className='text-xl font-semibold'>No Results</h3>
                                            <p>Try using keywords like rotator cuff, shoulder, treatment, etc...</p>
                                        </div>
                                    )}
                                </>
                            )}
                            ...
                            {activeTab === 'topics' && (
                                <>
                                    <h2 className="text-lg font-semibold mb-4">
                                        {activeCondition && activeCondition !== 'General' ? 'Related Topics' : 'Related Topics'}
                                    </h2>
                                    {loading && <SearchResultsSkeleton />}
                                    {!loading && activeCondition && activeCondition !== 'General' && sectionResults.length > 0 && (
                                        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
                                    ...
                                    {!loading && (!activeCondition || activeCondition === 'General') && topicResults.length > 0 && (
                                        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
                                            {topicResults.map((topic: any) => (
                                                <TopicCard
                                                    key={topic.id}
                                                    topic={topic}
                                                    link={topic.is_detailed
                                    ...
                                    {!loading && ((activeCondition && activeCondition !== 'General' && sectionResults.length === 0) || ((!activeCondition || activeCondition === 'General') && topicResults.length === 0)) && (
                                        <div className='flex flex-col w-full items-center'>
                                            <h3 className='text-xl font-semibold'>
                                                {activeCondition && activeCondition !== 'General' ? 'No Related Topics' : 'No Related Topics'}
                                            </h3>
                                        </div>
                                    )}
                                </>
                            ...
                            {activeTab === 'experts' && (
                                <>
                                    <h2 className="text-lg font-semibold mb-4">Associated Doctors</h2>
                                    {loading && <SearchResultsSkeleton />}
                                    {!loading && associatedDoctors.length > 0 ? (
                                        ...
                                        <div className='flex flex-col w-full items-center py-8'>
                                            <h3 className="text-xl font-semibold mb-2">No Associated Doctors Found</h3>
